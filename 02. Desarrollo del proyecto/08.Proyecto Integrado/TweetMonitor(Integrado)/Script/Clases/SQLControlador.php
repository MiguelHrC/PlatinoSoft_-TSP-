<?php
	...
	class SQLControlador{
		...
		public function ModificarUsuario($Usuarios)
		{
			$Mysql = new MySQLConector();
			$Mysql->Conectar();
			$DatosOriginales="SELECT * FROM usuarios  Where idUsuarios = " . $Usuarios->getidUsuarios() . ";";
			$DatosOriginalesC = $Mysql->Consulta($DatosOriginales);
			if ($Columnas = mysqli_fetch_array($DatosOriginalesC)) {
				if ($Columnas["Correo"]==$Usuarios->getCorreo() && $Columnas["Nombre"]!=$Usuarios->getNombre()){
